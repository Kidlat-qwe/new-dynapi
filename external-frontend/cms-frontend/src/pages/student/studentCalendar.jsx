...
const StudentCalendar = () => {
  ...
  const [filterOptions, setFilterOptions] = useState({ rooms: [] });
  ...
  }, [viewMode, selectedMonth, selectedWeekStart, roomFilter]);
  ...
  const fetchSchedules = async () => {
    ...
    try {
      ...
      const response = await apiRequest(`/calendar/student/schedules?${params.toString()}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">
            View your class schedules for {branchName}. Filter by room if needed.
          </p>
        </div>
      ...
      <div className="grid gap-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:grid-cols-2 lg:grid-cols-2">
        {viewMode === 'month' && (
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                    ...
                    {(holidaysByDate[cell.dateKey] || []).length > 0 ? null : (cell.events || []).length === 0 ? (
                      ...
                      cell.events.map((event) => (
                        <div
                          ...
                          className="rounded-lg border border-blue-100 bg-blue-50/60 p-1.5 text-[11px] leading-snug text-gray-700 cursor-pointer hover:bg-blue-100 hover:border-blue-200 transition-colors"
                        >
                          ...
                          {event.enrollment_phase && (
                            <p className="text-[10px] text-blue-600 font-medium">
                              Phase {event.enrollment_phase}
                            </p>
                          )}
                        </div>
                      ))
        ...
        {!loading && events.length === 0 && !error && (
          <div className="mt-4 rounded-xl border border-dashed border-gray-200 bg-gray-50 p-6 text-center text-sm text-gray-600">
            No schedules found for this {viewMode === 'week' ? 'week' : 'month'}. You may not be enrolled in any classes yet, or try adjusting your filters.
          </div>
        )}
      </div>
              ...
              {loadingClassDetails ? (
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="text-xs font-semibold uppercase tracking-wide text-gray-500">Your Enrollment Phase</label>
                      <p className="mt-1 text-sm text-gray-900">
                        {selectedEvent?.enrollment_phase ? `Phase ${selectedEvent.enrollment_phase}` : 'N/A'}
                      </p>
                    </div>
                    <div>
